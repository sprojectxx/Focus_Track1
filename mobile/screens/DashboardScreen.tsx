      ...
      >
        ...
        <View style={styles.sectionHeader}>
          <Text style={typography.h3}>
            {monthName} {currentYear} MATRIX
          </Text>
          <Text style={typography.caption}>MONTHLY EXECUTION GRID</Text>
        </View>

        {activeHabits.length === 0 ? (
          <EmptyState
            title="No Active Protocols"
            description="Create your first habit protocol to activate the monthly matrix."
          />
        ) : (
          <View style={styles.matrixContainer}>
            <View style={styles.matrixHeaderRow}>
              <View style={styles.stickyColumnHeader}>
                <Text style={typography.caption}>PROTOCOL</Text>
              </View>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={styles.daysHeaderRow}>
                  {Array.from({ length: daysInCurrentMonth }, (_, i) => i + 1).map((day) => {
                    const isTodayColumn = day === currentDay;
                    return (
                      <View
                        key={day}
                        style={[
                          styles.dayHeaderCell,
                          isTodayColumn && styles.dayHeaderCellToday,
                        ]}
                      >
                        <Text
                          style={[
                            typography.caption,
                            isTodayColumn && styles.dayHeaderCellTextToday,
                          ]}
                        >
                          {day}
                        </Text>
            ...
            {activeHabits.map((habit) => (
              <View key={habit.id} style={styles.matrixRow}>
                <View style={styles.stickyColumnCell}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={styles.cellsRow}>
                    {Array.from({ length: daysInCurrentMonth }, (_, i) => i + 1).map((day) => {
                      ...
                      const isCellToday = day === currentDay;
                      const isCellPast = day < currentDay;
                      const isCellFuture = day > currentDay;
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.matrixCell,
                            isCompleted && styles.matrixCellCompleted,
                            isCellToday && styles.matrixCellTodayBorder,
                            isCellFuture && styles.matrixCellFuture,
                          ]}
                          disabled={!isCellToday}
                          onPress={() => {
                            if (isCellToday) {
