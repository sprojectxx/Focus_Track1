...
BEGIN
  ...
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email), COALESCE(NEW.raw_user_meta_data->>'avatar_url', ''), random_creed, 'UTC', FALSE)
...
BEGIN
  ...
  END IF;
  SELECT p.timezone INTO user_tz FROM public.profiles AS p
    WHERE p.id = p_user_id AND p.timezone IS NOT NULL AND p.timezone <> ''
      AND EXISTS (SELECT 1 FROM pg_timezone_names AS tz WHERE tz.name = p.timezone);
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  local_today DATE; row_user_id UUID; deleting_ids TEXT[];
  target_user_id UUID; target_schedule_days INT[]; target_is_archived BOOLEAN;
  target_archived_at TIMESTAMPTZ; target_archived_intervals JSONB;
  ft_day_index INT; interval_record JSONB; arch_ymd TEXT; rest_ymd TEXT; date_str TEXT;
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF target_user_id IS NULL THEN RAISE EXCEPTION 'Habit % does not exist.', NEW.habit_id; END IF;
    IF target_user_id <> NEW.user_id THEN RAISE EXCEPTION 'Habit % does not belong to user %.', NEW.habit_id, NEW.user_id; END IF;
    ...
    IF target_archived_intervals IS NOT NULL AND jsonb_array_length(target_archived_intervals) > 0 THEN
      FOR interval_record IN SELECT * FROM jsonb_array_elements(target_archived_intervals) LOOP
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.user_id <> NEW.user_id THEN RAISE EXCEPTION 'Cannot change habit log user_id.'; END IF;
    IF OLD.habit_id <> NEW.habit_id THEN RAISE EXCEPTION 'Cannot change habit log habit_id.'; END IF;
